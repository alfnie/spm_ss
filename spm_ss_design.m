...
%    ss.Localizer_conjunction_type   (for multiple localizers) type of conjunction ('and','or','min','max','prod','sum','omnibus'). Option 'and': intersection of individually-thresholded localizer masks; 'or': union of individually-thresholded localizer masks; 'min': thresholded minimum p-value across localizer effects; 'max': thresholded maximum p-value across localizer effects; 'prod': thresholded product of p-values across localizer effects; 'sum': thresholded sum of p-values across localizer effects; 'omnibus': p-value computed from omnibus test across localizer effects; (default 'and') 
%                                    note: for 'min/max/prod/sum/omnibus' options only 'percentile-whole-brain','percentile-ROI-level','Nvoxels-whole-brain','Nvoxels-ROI-level' Localizer_thr_type options are available; Localizer_thr_p and Localizer_thr_type values must take a single value (not a separate value per localizer in conjunction); and localizers with -not prefixes will use 1-p instead of p-values in the conjunction operation
...
if ~ss.files_selectmanually&&(ss.askn>1||~isfield(ss,'Localizer_thr_type')||numel(ss.Localizer_thr_type)<size(ss.Localizer_contrasts,2)||~isfield(ss,'Localizer_thr_p')||numel(ss.Localizer_thr_p)<size(ss.Localizer_contrasts,2)),
    ...
    if ss.askn,
        ...
        if size(ss.Localizer_contrasts,2)>1
            ...
            disp(str);
            sstype3options={'and','or','min','max','prod','sum','omnibus'};
            sstype3=spm_input(str,posstr,'m','and|or|min|max|prod|sum|omnibus',[],strmatch(lower(ss.Localizer_conjunction_type),sstype3options,'exact'));posstr='+1';
